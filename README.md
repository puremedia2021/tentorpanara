...
Sistem operasional payroll tentor berbasis Cloudflare Workers + D1 + R2, dengan alur:
...
4. Masing-masing pihak membuka link, upload foto bukti (wajah saat belajar), dan klik "Acc".
5. Sesi yang sudah di-acc oleh **kedua** pihak otomatis masuk ke `/rekap.html` sebagai dasar payroll.

## Struktur

- `schema.sql` — skema database D1 (paket, tentor, siswa, sesi)
- `src/index.js` — Worker: semua endpoint `/api/*`
- `public/` — halaman statis (admin, input sesi, approval, rekap)
- `wrangler.toml` — konfigurasi binding D1, R2, dan static assets
...
Buat bucket R2 untuk foto:

```bash
npx wrangler r2 bucket create tentor-payroll-photos
```
